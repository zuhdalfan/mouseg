 ...
 *(.ARM.exidx* gnu.linkonce.armexidx.*)
 ...
 service_area : { _service_list_start = .; KEEP(*(SORT(._service.static.*_?_*))); KEEP(*(SORT(._service.static.*_??_*))); KEEP(*(SORT(._service.static.*_???_*))); KEEP(*(SORT(._service.static.*_????_*))); KEEP(*(SORT(._service.static.*_?????_*))); _service_list_end = .;; } > FLASH
 sw_isr_table :
 {
  . = ALIGN(4);
